![image-20220510223233595](assets/springsecurity/image-20220510223233595.png)
...
我们在JavaWeb阶段已经了解了`Session`和`Cookie`的机制，在一开始的时候，服务端会给浏览器一个名为`JSESSION`的Cookie信息作为会话的唯一凭据，只要用户携带此Cookie访问我们的网站，那么我们就可以认定此会话属于哪个浏览器。因此，只要此会话的用户执行了登录操作，那么就可以随意访问个人信息等内容。
...
接着创建`Initializer`来配置Web应用程序，建议单独创建一个初始化包`initial`：
...
创建Mvc配置类,和mvc有关的配置都是在这个里面，建议新建一个配置包`config`：
...
接着我们需要配置`SpringSecurity`，与Mvc一样，需要一个初始化器：
...
接着我们需要再创建一个配置类用于配置`SpringSecurity`，这个配置类是放在`MvcInitializer`的`getRootConfigClasses`根容器中的，因为这个SpringSecurity是和业务层相关的：

> 后面学完看一下为何这个SpringSecurity明明是Security，怎么不走Security的初始化类SecurityInitializer
...
这样，`SpringSecurity`的配置就完成了，我们再次运行项目，会发现无法进入的我们的页面中，无论我们访问哪个页面，都会重定向进入到`SpringSecurity`为我们提供的一个默认登录页面，之后我们会讲解如何进行配置。
...
形参`AuthenticationManagerBuilder`是主要处理用户登录的，这里有几个`configure`，要注意区分
...
![image-20220511085534092](data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 738"></svg>)
...
然后我们需要创建一个Service实现，实现的是`UserDetailsService`，它支持我们自己返回一个`User`实现了`UserDetails`的对象，我们只需直接返回一个包含数据库中的用户名、密码等信息的UserDetails即可，SpringSecurity会自动进行比对。
...
将代码添加`@Service`注解加入bean容器，@Service和@Component是一样的，针对不同特点的类
...
最后再修改一下Security配置，将之改为通过数据库认证的方式，而不是直接认证：

```java
// 将UserAuthService注入进来
@Resource
...
接着我们就可以将我们自己的页面替换掉默认的页面了，我们需要在`SecurityConfiguration`中重写另一个方法来实现：
...
```java
.and()	// and方法表示重新回到HttpSecurity对象，重新开始新一轮配置，一般用于两种不同的配置组，暂时这么理解
...
需要配置登陆页面的地址和登陆请求发送的地址，`loginPage`自定义登陆页面填写为`/login`，`loginProcessingUrl`登陆请求地址为`/doLogin`，登陆页面需要我们自己去编写`Controller`设置`mapping`来实现，登陆请求提交处理由SpringSecurity提供，只需要写路径就可以了。

`loginProcessingUrl`可以不填写，则默认是和loginPage的/login一样，这个其实没什么用，因为这些逻辑都是SpringSecurity处理的，填写这个只是显示提交的地址而已，无论如何都会提交到SpringSecurity，loginPage自定义登陆也可以不写，则使用SpringSecurity默认的登陆页面，两个都不写则是默认页面默认提交地址。

> loginPage和loginProcessingUrl地址是严格区分大小写的，因为大小写弄错搞了几个小时才解决!
...
.successForwardUrl("/index")    //登陆成功后跳转的页面，也可以通过Handler实现高度自定义
.failureForwardUrl("/login")    //登陆失败后跳转的页面，也可以通过Handler实现高度自定义
.passwordParameter("password")       //表单password的name
.usernameParameter("username")       //表单username的name
```
...
接着我们需要配置`SecurityConfiguration`，决定哪些角色可以访问哪些页面：
...
1. 首先要有含有`role`权限的用户表，然后创建对应的实体表
2. 创建基于这个user表的映射`mapper`
3. 创建实现`UserDetails`的`User`对象，保存用户的信息，当然也可以将这些直接配置在下面的`configure`中(直接认证)
4. 将这个`service`实现类保存到`SecurityConfiguration`的`configure`配置中，这样就完成了读取用户信息的操作
...
public UserDetails loadUserByUsername(String s) throws UsernameNotFoundException {
    ...
    return User
            .withUsername(user.getUsername())
            .password(user.getPassword())
            // 这个字符串就是数据库的字符串，也可以是数组new ArrayList<GrantedAuthority>()
            // 字符串有哪些，这个用户就有哪些权限，然后上面配置有哪些权限即可
...
除了直接配置以外，我们还可以以注解形式直接配置，首先需要在配置类（注意这里是在Mvc的配置类上添加，因为这里只针对Controller进行过滤，所有的Controller是由Mvc配置类进行注册的，如果需要为Service或其他Bean也启用权限判断，则需要在Security的配置类上添加，由谁管就在哪里添加这个注解）上开启：
...
除了Controller以外，只要是由Spring管理的Bean都可以使用注解形式来控制权限，只要不具备访问权限，那么就无法执行方法并且会返回403页面，只要在执行流程之内，遇到这个权限验证都会进行判断，无论是什么方法
...
我们还可以使用`@PreFilter`和`@PostFilter`对执行前的集合类型的参数和方法执行后的返回值进行过滤。

比如：

```java
@PreFilter("filterObject.equals('lbwnb')")   //filterObject代表集合中每个元素，只有满足条件的元素才会留下
...
SpringSecurity为我们提供了一种优秀的实现，它为每个已经登陆的浏览器分配一个携带Token的Cookie，并且此Cookie默认会被保留14天，只要我们不清理浏览器的Cookie，那么下次携带此Cookie访问服务器将无需登陆，

Cookes的安全性问题，后面学习了更多知识后再研究

直接继续使用之前登陆的身份，这样显然比我们之前的写法更加简便。并且我们需要进行简单配置，即可开启记住我功能：
...
再次打开index页面，就不需要再次登录了，访问的时候自动携带了cookie信息，这个信息是springsecurity在渲染的时候就设置好的，只要开起了`remember()`方法

当然点击退出登录，springsecurity会自动清除这个cookie，因为`logout`方法也是它自身的方法
...
```java
// 引用注入到当前页面，相当于我引用我自己，两个都是在这个页面配置的，但是只能这样，不可能直接new一个，因为只能通过容器的形式注入和新建对象
@Resource
...
public String index(){
    new Thread(() -> {   //创建一个子线程去获取
        ...
        System.out.println(user.getUsername());
        System.out.println(user.getAuthorities()); 	// [ROLE_user] 有的地方会加ROLE前缀，配置的时候又不加，要注意一下
    }).start();
    return "index";
}
```
...
因此，如果上述情况需要在子线程中获取，那么需要修改`SecurityConfiguration`文件的`SecurityContextHolder`的存储策略，在初始化的时候设置：
