![image-20220509231827873](assets/springmvc/image-20220509231827873.png)
...
![image-20220509231836245](assets/springmvc/image-20220509231836245.png)
...
![image-20220509231844195](assets/springmvc/image-20220509231844195.png)
...
![image-20220509231855235](assets/springmvc/image-20220509231855235.png)
...
![image-20220510084952373](assets/springmvc/image-20220510084952373.png)
...
public class WebConfiguration {
  ...
  //我们需要使用ThymeleafViewResolver作为视图解析器，并解析我们的HTML页面
    ...
    public SpringResourceTemplateResolver templateResolver(){
        ...
        resolver.setCharacterEncoding("UTF-8");   //编码格式是重中之重
        resolver.setPrefix("/WEB-INF/tempalate/");   //需要解析的HTML页面文件存放的位置，从WEB-INF开始
        return resolver;
    }
...
现在我们就完成了视图解析器的配置，我们接着来创建一个Controller，创建Controller也非常简单，只需在一个类上添加一个`@Controller`注解即可，它会被Spring扫描并自动注册为Controller类型的Bean，然后我们只需要在类中编写方法用于处理对应地址的请求：
...
同时在`WebConfiguration`将controller目录扫描到Bean
...
我们的页面中可能还会包含一些静态资源，比如js、css，因此这里我们还需要配置一下，让静态资源通过Tomcat提供的默认Servlet进行解析，我们需要让**配置类实现一下**`WebMvcConfigurer`接口，同时添加`@EnableWebMvc`注解这样在Web应用程序启动时，会根据我们重写方法里面的内容进行进一步的配置：
...
同时除了返回`ModelAndView`之外，还可以以返回`String`的形式，创建`mapping`，一般常用这种形式，更加简便：
...
直接添加`HttpServletRequest`为形式参数即可，SpringMVC会自动传递该请求原本的`HttpServletRequest`对象，同理，我们也可以添加`HttpServletResponse`作为形式参数，通常使用HttpServletResponse来作为响应形式参数，当然HttpServletRequest通常不使用，因为默认就已经注入到参数里面了，甚至可以直接将HttpSession也作为参数传递：
...
注意必须携带set方法或是构造方法中包含所有参数，请求参数会自动根据类中的字段名称进行匹配，请求的参数会自动装载进`user`对象：
...
请注意，一旦设置了请求参数，而且没有设置非必须输入和默认值的话，则这个请求必须有这些请求参数，否则就`404`无法找到这个请求
...
接着将其注册进`WebConfiguration`为Bean，注意这里需要添加`@RequestScope`或是`@SessionScope`表示此Bean的Web作用域：
...
接着我们将其自动注入到Controller中，然后就可以使用了：
...
![image-20220510160455646](assets/springmvc/image-20220510160455646.png)
...
也就是说，处理前和处理后，包含了真正的请求映射的处理，在整个流程结束后还执行了一次`afterCompletion`方法，其实整个过程与我们之前所认识的Filter类似，不过在处理前这个阶段，我们只需要返回`true`或是`false`表示是否被拦截即可，而不是再去使用FilterChain进行向下传递。
...
前面介绍了仅仅只有一个拦截器的情况，我们接着来看如果存在多个拦截器会如何执行，我们以同样的方式创建二号拦截器，新建一个拦截器：
...
和多级Filter相同，在处理之前，是按照顺序从前向后进行拦截的，但是处理完成之后，就按照倒序执行处理后方法，而完成后是在所有的`postHandle`执行之后再同样的以倒序方式执行，流程类似栈，**先进后出**，同时处理之前，处理，处理之后，完成之后，所有拦截器是在一个时间阶段执行的，不会存在一个拦截器的完成之后跑到另外一个的处理中这个阶段去执行。
...
我们发现，与单个拦截器的情况一样，一旦第一个拦截器返回false，那么之后无论有无拦截器，都不再继续。

但是如果第一个拦截器放行，第二个拦截器返回false，那么执行情况如下：
...
可以发现，二号拦截器返回了false，那么二号拦截器的所有处理中、处理之后、完成之后都不会执行，但是一号拦截器还是执行了完成之后，是因为一号已经返回true放行，然后是在放行后发生了其它异常或者被拦截，无论如何一号已经放行，被二号拦截，那么处理之后肯定就执行不到了(出现异常或者被其它拦截器拦截都不会执行处理之后)，但是一号的完成之后一定会执行，**只要放行就会执行完成之后**。
...
重复引用问题：重复引用是在一个集合中重复add两个相同的对象元素，那么第二次加入的元素就不会显示具体的对象，而是显示的`{"$ref":"$[0]"}`,表示第二个元素是引用对象，引用的$跟对象的[0]第一个元素

而循环引用更加复杂，表示的是两个对象，我引用了你，你引用了我

一般不会出现这种问题，应该尽量避免循环引用对象，等实际出现的时候再研究。

当出现循环引用时，会按照以下语法来解析：

![image-20220510194701065](assets/springmvc/image-20220510194701065.png)
...
这里我们修改了`produces`的值，将返回的内容类型设定为`application/json`，表示服务器端返回了一个JSON格式的数据（当然不设置也行，也能展示，这样是为了规范）然后我们在方法上添加一个`@ResponseBody`表示方法返回（也可以在类上添加`@RestController`表示此Controller默认返回的是字符串数据）的结果不是视图名称而是直接需要返回一个字符串作为页面数据，这样，返回给浏览器的就是我们直接返回的字符串内容，添加@RespenseBody就表示，给客户端返回什么，客户端就收到什么，而不是返回一个页面渲染。
...
![AJAX](assets/springmvc/ajax-yl.png)
...
public String upload(@RequestParam CommonsMultipartFile file) throws IOException {
    ...
    File fileObj = new File("test.html");
  	// 也可以自定义上传路径，注意是绝对路径
    // File fileObj = new File("/Users/easylee/Downloads/test.html");
...
要注意，文件上传下载的默认目录是`tomcat`的`bin`目录开始
...
public void download(HttpServletResponse response){
  ...
  // 获取getOutputStream 文件输出流
  try(OutputStream stream = response.getOutputStream();
      // 获取文件输入流
      InputStream inputStream = new FileInputStream("/Users/easylee/Downloads/test.html")){
    // 将输入流中的数据写入到输出流中，IOUtils是导入apache commons-fileupload包时自动导入的
...
![image-20220510214643982](data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 946 532"></svg>)
...
![image-20220510214653271](assets/springmvc/image-20220510214653271.png)
