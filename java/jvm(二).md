...
![image-20220520004532039](assets/jvm(二)/image-20220520004532039.png)
...
![image-20220131110349472](assets/jvm(二)/008i3skNly1gywoc0w7ouj30xm0hy401.jpg)
...
![image-20220131142625842](assets/jvm(二)/008i3skNly1gywucw6rcyj30ws0gyq4h.jpg)

接着我们继续往下，到了` 0: invokestatic  #2                  // Method a:()I`时，需要调用方法`a()`，这时当前方法就不会继续向下运行了，而是去执行方法`a()`，那么同样的，将此方法也入栈，注意是放入到栈顶位置，`main`方法的栈帧会被压下去：

![image-20220131143641690](assets/jvm(二)/008i3skNly1gywuhfjok5j30v40g875z.jpg)

这时，进入方法a之后，又继而进入到方法b，最后在进入c，因此，到达方法c的时候，我们的虚拟机栈变成了：

![image-20220131144209743](assets/jvm(二)/008i3skNly1gywun3qnp6j30zq0h6jtq.jpg)
...
![image-20220131144955668](assets/jvm(二)/008i3skNgy1gywxbv24qlj30tk0giwg2.jpg)
...
![image-20220201140516096](assets/jvm(二)/008i3skNly1gyxz722qmjj31520mmgo9.jpg)
...
![image-20220201141848804](assets/jvm(二)/008i3skNly1gyy0jttx6mj318g0iswgd.jpg)
...
![image-20220201142710405](assets/jvm(二)/008i3skNly1gyy0jrivm4j318k0jcq4q.jpg)
...
![image-20220201145204505](assets/jvm(二)/008i3skNly1gyy0jx0o6gj31fk0la41e.jpg)

所以上述结果中得到的依然是两个`true`。在JDK1.7之后，稍微有一些区别，在调用`intern()`方法时，当常量池中没有对应的字符串时，不会再进行复制操作，而是将其直接修改为指向当前字符串堆中的的引用：

![image-20220201144747139](assets/jvm(二)/008i3skNly1gyy0jyvnstj31f20k0di6.jpg)
...
![image-20220201202346882](assets/jvm(二)/008i3skNly1gyya4xksfzj31cm0u0dk2.jpg)
...
![image-20220201203157213](assets/jvm(二)/008i3skNly1gyyaddef66j31vo0u0jwq.jpg)
...
![image-20220222084649786](assets/jvm(二)/e6c9d24egy1gzm008b8j2j21ik0tagpd.jpg)
...
![image-20220222125507229](assets/jvm(二)/e6c9d24egy1gzm76iz1mzj217s0ggwgc.jpg)

一旦已经存在的根节点不满足存在的条件时，那么根节点与对象之间的连接将被断开。此时虽然对象1仍存在对其他对象的引用，但是由于其没有任何根节点引用，所以此对象即可被判定为不再使用。比如某个方法中的局部变量引用，在方法执行完成返回之后：

![image-20220222130350950](assets/jvm(二)/e6c9d24egy1gzm7ohrh9kj21bg0heacd.jpg)

这样就能很好地解决我们刚刚提到的循环引用问题，我们再来重现一下出现循环引用的情况：

![image-20220222130903349](assets/jvm(二)/e6c9d24egy1gzm7ofteqej215a0a00tk.jpg)

可以看到，对象1和对象2依然是存在循环引用的，但是只有他们各自的GC Roots断开，那么就会变成下面这样：

![image-20220222131219350](assets/jvm(二)/e6c9d24egy1gzm7of7nnlj21740dq75u.jpg)
...
![image-20220222141854678](assets/jvm(二)/e6c9d24egy1gzm9o931z4j21n40letdm.jpg)
...
![image-20220222151708141](assets/jvm(二)/e6c9d24egy1gzmbaa6eg9j217a0ggta0.jpg)

那么它是如何运作的呢？

首先，所有新创建的对象，在一开始都会进入到新生代的Eden区（如果是大对象会被直接丢进老年代），在进行新生代区域的垃圾回收时，首先会对所有新生代区域的对象进行扫描，并回收那些不再使用对象：

![image-20220222153104582](assets/jvm(二)/e6c9d24egy1gzmbyo48r0j21i20cqq4l.jpg)
...
![image-20220222154032674](assets/jvm(二)/e6c9d24egy1gzmbyn34yfj21gk0d4gn5.jpg)
...
![image-20220222154828416](assets/jvm(二)/e6c9d24egy1gzmc6v1nzcj21h60d2q4l.jpg)
...
![image-20220222162536616](assets/jvm(二)/e6c9d24egy1gzmd9jj8djj21m20gktav.jpg)
...
![image-20220222205605690](assets/jvm(二)/e6c9d24ely1gzml30209wj21u80ren3q.jpg)
...
![image-20220222165709034](assets/jvm(二)/e6c9d24egy1gzme6btluwj21e40c0760.jpg)
...
![image-20220222210942507](assets/jvm(二)/e6c9d24ely1gzmlh5aveqj21ti0u079c.jpg)
...
![image-20220222213208681](assets/jvm(二)/e6c9d24ely1gzmm4g8voxj21vm08ywhj.jpg)
...
![image-20220223104605648](assets/jvm(二)/e6c9d24ely1gzn92k8ooej21ae0bc75m.jpg)
...
![image-20220223111344962](assets/jvm(二)/e6c9d24ely1gzn9vbvb0mj21c20c00uc.jpg)
...
![image-20220223112108949](assets/jvm(二)/e6c9d24ely1gzna31mo1qj21cs0ckjt3.jpg)
...
![image-20220223114019381](assets/jvm(二)/e6c9d24ely1gznamys2bdj21as0co404.jpg)
...
![image-20220223123636582](assets/jvm(二)/e6c9d24ely1gznc9jvdzdj21f40eiq4g.jpg)

它的回收过程与CMS大体类似：

![image-20220223123557871](assets/jvm(二)/e6c9d24ely1gznc8vqqqij21h00emwgt.jpg)
...
![image-20220223130536357](assets/jvm(二)/e6c9d24ely1gznd3pdzvyj21q20fcacr.jpg)

因此在JDK8时直接将本地内存作为元空间（**Metaspace**）的区域，物理内存有多大，元空间内存就可以有多大，这样永久代的空间分配问题就讲解了，所以最终它变成了这样：

![image-20220223125137512](assets/jvm(二)/e6c9d24ely1gzncp6mhikj21ik0migqv.jpg)
