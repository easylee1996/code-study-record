...
![image-20220520003326293](assets/jvm(一)/image-20220520003326293.png)
...
![image-20220520003416813](assets/jvm(一)/image-20220520003416813.png)

也正是得益于这种统一规范，除了Java以外，还有多种JVM语言，比如Kotlin、Groovy等，它们的语法虽然和Java不一样，但是最终编译得到的字节码文件，和Java是同样的规范，同样可以交给JVM处理。

![image-20220520004039932](assets/jvm(一)/image-20220520004039932.png)
...
![image-20220520004106422](assets/jvm(一)/image-20220520004106422.png)
...
![image-20220520003445054](assets/jvm(一)/image-20220520003445054.png)
...
![image-20220520004132576](assets/jvm(一)/image-20220520004132576.png)
...
![image-20220520004218501](assets/jvm(一)/image-20220520004218501.png)

点击`应用`即可，接着打开运行配置，添加一个新的自定义配置：

![image-20220520004209470](assets/jvm(一)/image-20220520004209470.png)
...
![image-20220520004235233](assets/jvm(一)/image-20220520004235233.png)
...
![image-20220520003541597](assets/jvm(一)/image-20220520003541597.png)

点击右上角调试按钮，可以成功进行调试：

![image-20220520003547931](assets/jvm(一)/image-20220520003547931.png)
...
![image-20220520004404691](assets/jvm(一)/image-20220520004404691.png)

还是以我们之前编写的测试类进行，首先来到调用之前，我们看到主方法执行之前，控制台没有输出任何内容，接着我们执行此函数，再来观察控制台的变化：

![image-20220129211450939](assets/jvm(一)/008i3skNly1gyux4w5322j31zt0u0afp-20220520004333616.jpg)

可以看到，主方法执行完成之后，控制台也成功输出了Hello World！

继续下一步，整个Java程序执行完成，得到退出状态码`0`：

![image-20220129211540210](assets/jvm(一)/008i3skNly1gyux4ydghaj31bk0eimy7-20220520004333616.jpg)

成功验证，最后总结一下整个执行过程：

![image-20220520004508256](assets/jvm(一)/image-20220520004508256.png)
...
![img](assets/jvm(一)/008i3skNly1gyux540wn7j30xc0h1q47-20220520004333636.jpg)
...
![image-20220129222858105](assets/jvm(一)/008i3skNly1gyux58pg32j31ag0smjv5-20220520004333609.jpg)
